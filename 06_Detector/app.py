...
def prediction():
    ...
    if request.method == 'POST':
        ...
        # First model
        template_data['pipeline1']['prediction'] = str(title_pipeline.predict([text])[0])
        if template_data['pipeline1']['prediction'] == 'True':
            template_data['pipeline1']['percent'] = round(title_pipeline.predict_proba([text])[0][1] * 100, 2)
            template_data['pipeline1']['prediction_color'] = 'green'
        else:
            template_data['pipeline1']['percent'] = round(title_pipeline.predict_proba([text])[0][0] * 100, 2)
